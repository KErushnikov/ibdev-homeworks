...
* Ответ на оба вопроса - использвуется метод аутентификации "trust". [Аутентификация Trust](https://postgrespro.ru/docs/postgrespro/12/auth-trust).
![](./pic/screenshot1.JPG)
...
1. Утверждение верно. Пароль роли `app` хранится в поле `rolpassword`. Сам пароль `md50d06d8eb2a5f36ab73d83481186543d5`, где `md5` - функция хэширования, а `0d06d8eb2a5f36ab73d83481186543d5` - хэшированный пароль.
![](./pic/screenshot2.JPG)
...
1.  Использвуется метод аутентификации "trust". [Аутентификация Trust](https://postgrespro.ru/docs/postgrespro/12/auth-trust).
2.  Использвуется метод аутентификации "reject", то есть всем подключения не с `localhost' будет отказано.
![](./pic/screenshot3.JPG)
3. Утверждение верно. Пароль роли `app` хранится в поле `rolpassword`. Сам пароль `md50d06d8eb2a5f36ab73d83481186543d5`, где `md5` - функция хэширования, а `0d06d8eb2a5f36ab73d83481186543d5` - хэшированный пароль.
4. Все указанные в вопросе поля для пользователя `"app"` не изменились и имеют значение `"t"`, т.е. `"True"`. Назначения:
   *  `rolsuper` - определяет, является ли указанная учетная запись учетной записью суперпользователя.
   *  `rolcreaterole` - определяет, может ли указанная учетная запись создавать другие учетные записи
   *  `rolcreatedb` - определяет, может ли  указанная учетная запись создавать "базы данных".
   *  `rolbypassrls` - определяет, будут ли для роли игнорироваться все политики защиты на уровне строк (RLS).
![](./pic/screenshot4.JPG)
5. Мы подключаемся с помощью `psql` внутри контейнера, т.е. с локальной машины, а для локальной машины метод аутентификации выставлен `"trust"`. Строка `host all all all reject` идет после строк в файле `pg_hba.conf`, разрешающих подключение.
...
Изучив [CIS PostgreSQL 12 Benchmark v1.1.0](https://workbench.cisecurity.org/benchmarks/11861) можно сделать следующие выводы:
1. Методы аутентификации `trust`, `password`, `ident` не должны использоваться для удаленных подключений. "METHODs trust, password, and ident are not to be used for remote logins."
2. Хотя метод аутентификации `md5` очень популярен и распространен, но уязвим. Таким образом рекомендуется использовать метод `scram-sha-256` вместо `md5`. "METHOD md5 is the most popular and can be used in both encrypted and unencrypted sessions, however, it is vulnerable to packet replay attacks. It is recommended that scram-sha-256 be used instead of md5."
![](./pic/screenshot5.JPG)
