...
4\. Аутентифицируйтесь с помощью команды `AUTH app password`, где вместо `password` подставьте пароль, полученный из хеша. Вам нужно будет вспомнить, как доставать из хеша пароли. Пароль `secret`
![](./pic/hash.JPG)
...
Результаты вывода команд прописаны в задании.
...
Записи из лога событий ACL для событий:
- попытка входа с неверным паролем;
  ``` text
  2)  1) "count"
    2) (integer) 1
